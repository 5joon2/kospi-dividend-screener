"""정성평가 입력 페이지 — 규모(대형/중형/소형주)별 상위 30종목, 공유 비밀번호로 보호된 편집 UI."""
...
quant_df["ticker"] = quant_df["ticker"].str.zfill(6)
if "market_cap" not in quant_df.columns:
    quant_df["market_cap"] = 0

# KRX 코스피 대형/중형/소형주 지수와 같은 기준(1~100위/101~300위/301위~) — 메인 화면(app.py)과 동일 로직
...
def render_group_editor(group: str) -> None:
    ...
    edited = st.data_editor(
        ...
        key=f"qual_editor_{group}",
    )

    if st.button(f"{group} 저장", type="primary", disabled=not editor_name, key=f"save_{group}"):
